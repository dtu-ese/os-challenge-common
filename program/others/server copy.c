...
typedef struct
{
    ...
    CacheStatus status;
    pthread_cond_t condition; // To notify waiting threads
} HashCacheEntry;
...
{
    pthread_mutex_lock(&cache_mutex);
    cache[cache_size].number = 0; // Not solved yet
    memcpy(cache[cache_size].hash, target_hash, SHA256_DIGEST_LENGTH);
    cache[cache_size].status = IN_PROGRESS;
    pthread_cond_init(&cache[cache_size].condition, NULL); // Initialize condition variable
    cache_size++;
    pthread_mutex_unlock(&cache_mutex);
}
...
uint64_t bruteForce(unsigned char target_hash[], uint64_t start, uint64_t end)
{
    ...
    // If not in cache, add as "in-progress"
    ...
    for (uint64_t num = start; num < end; num++)
    {
        ...
        if (memcmp(hash, target_hash, SHA256_DIGEST_LENGTH) == 0)
        {
            ...
            pthread_mutex_lock(&cache_mutex);
            // Update cache as solved and notify all waiting threads
            for (size_t i = 0; i < cache_size; i++)
            {
                if (memcmp(cache[i].hash, target_hash, SHA256_DIGEST_LENGTH) == 0)
                {
                    ...
                    pthread_cond_broadcast(&cache[i].condition); // Notify waiting threads
                    break;
                }
            }
            pthread_mutex_unlock(&cache_mutex);
    ...
    for (size_t i = 0; i < cache_size; i++)
    {
        if (memcmp(cache[i].hash, target_hash, SHA256_DIGEST_LENGTH) == 0)
        {
            cache[i].status = NONE;                      // Mark the cache entry as failed
            pthread_cond_broadcast(&cache[i].condition); // Notify waiting threads
            break;
        }
    }
